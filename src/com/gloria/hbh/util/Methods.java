...
/**
 * 文 件 名 : Methods 创 建 人： gejian 日 期：2012-8-7 修 改 人：gejian 日 期：2012-8-7 描
 * 述：程序的方法类
 */
@TargetApi(5)
public class Methods {
	...
	public static String setNewContent(String contentstring) { // 替换img标签
		...
		Pattern pattern2 = Pattern.compile("<img[^<>]+src=['\"]http://[^<>]+>", Pattern.CASE_INSENSITIVE); // 匹配带有<img
																											// src="http://
																											// >
		...
		tmpString = tmpString.replace(BaseConstants.FACE_IMG_CONTAIN_PATH,
				BaseConfig.url + BaseConstants.FACE_IMG_CONTAIN_PATH); // 替换表情的img标签
	...
	// //替换表情的图标：
	// Pattern pattern3 =
	// Pattern.compile("(:em(\\d{1,4}):)",Pattern.CASE_INSENSITIVE); //匹配带有<img
	// src="http:// >
	...
	// GlobalSource.url+"static/image/smiley/"); //替换表情的img标签
	// return tmpString;
	// }
