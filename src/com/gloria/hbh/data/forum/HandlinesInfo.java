...
public class HandlinesInfo extends BaseData {
	...
	public static HandlinesInfo getHandlinesInfoFromJsonObject(JsonObject jsonObject_imgs,
			JsonObject jsonObject_lists) {
		...
		try {
			...
			if (jsonArray_imgs_list != null) {
				for (int i = 0; i < jsonArray_imgs_list.size(); i++) {
					...
					handlinesInfo.getHandlinesImg()
							.add(HandlinesBasicInfo.getHandlinesBasicInfoFromJsonObject(jsonObject_imgs_list));
				}
			}
			if (jsonArray_lists_list != null) {
				for (int i = 0; i < jsonArray_lists_list.size(); i++) {
					...
					handlinesInfo.getHandlinesList()
							.add(HandlinesBasicInfo.getHandlinesBasicInfoFromJsonObject(jsonObject_lists_list));
				}
			}
	...
	public static ArrayList<HandlinesBasicInfo> getHandlinesInfoFromParseObject(List<ParseObject> images) {
