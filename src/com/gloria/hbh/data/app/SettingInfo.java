...
/**
 * 文 件 名 : SettingsInfo 创 建 人： gejian 日 期：2012-8-7 修 改 人：gejian 日 期：2012-8-7 描
 * 述：程序的配置数据结构
 */
			...
			if (root.get(i) instanceof HashMap) {
				...
				for (int j = 0; j < setting.size(); j++) {
					if (setting.get(j) instanceof HashMap) {
						...
						// if(settingInfo2.getType() ==
						// SettingTypeConstants.SETTING_USE_BAIDU_INPUT){
						// continue;
						// }
						settingInfo.getSettings().add(SettingInfo.getSettingInfo(setting_));
					}
					// else if(setting.get(j) instanceof ArrayList){
					// ArrayList<Object> setting_ = (ArrayList<Object>)
					// setting.get(j);
					// }
				}
