        ...
        text_title.setText(title);
	 	...
	 	if(imageUrl != null && !imageUrl.equals("")){ 
	 	    ...
	 	    try
            {
               ...
               {
                   text_title.setText("");
                   image.setBackgroundDrawable(new BitmapDrawable((BitmapFactory.decodeStream(getResources().getAssets().open(imageUrl.substring(9))))));
               }
               else { 
	 	...
	 	if(type == SubTabInfoTypeConstants.SUBCATE_ACTIVITY
	 			|| type == SubTabInfoTypeConstants.SUBCATE_TOURIST){
...
//		mediaTypes.add(MediaType.WEIXIN_TIMELINE);
		...
		if(imageUrl != null && !imageUrl.equals("")){
			mImageContent = new ShareContent(
					title,
					TextUtils.getWeiBoContent(description),
					"http://www.hbhsc.net/",
					Uri.parse(imageUrl));
		...
		SocialShare.getInstance(this, clientID)
			.show(
				getWindow().getDecorView(), mImageContent, UIWidgetStyle.DEFAULT,
				mDefaultListener);
	}
			...
			public void run() {
				if(!isFinishing()) {
					if(mAlertDialog == null) {
						mAlertDialog = new android.app.AlertDialog.Builder(Activity_Detail.this).setPositiveButton(R.string.ok, null).create();
