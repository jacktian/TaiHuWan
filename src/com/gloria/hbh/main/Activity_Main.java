...
public class Activity_Main extends SlidingFragmentActivity {
	...
	private void getMapFragment() {
		...
		fragmentTransaction.replace(R.id.content_main, mapFragment == null ? new MapFragment("地图导览") : mapFragment,
				text);
	...
	private void getMallFragment() {
		...
		fragmentTransaction.replace(R.id.content_main, mallFragment == null ? new MallFragment("商城") : mallFragment,
				text);
		fragmentTransaction.addToBackStack(text);
		fragmentTransaction.commit();
	}
