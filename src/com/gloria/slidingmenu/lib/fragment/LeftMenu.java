			...
			public boolean onChildClick(ExpandableListView parent, View v, final int groupPosition,
				...
				if (leftMenuAdapter.getGroup(groupPosition).getTitle().equals("辅展区")) {
					...
					intent.putExtra("name",
							leftMenuAdapter.getGroup(groupPosition).getSublist().get(childPosition).getTitle());
					intent.putExtra("detail",
							leftMenuAdapter.getGroup(groupPosition).getSublist().get(childPosition).getDetail());
					intent.putStringArrayListExtra("images",
							leftMenuAdapter.getGroup(groupPosition).getSublist().get(childPosition).getImages());
				...
				new Handler().postDelayed(new Runnable() {
					public void run() {
						...
						String location = leftMenuAdapter.getGroup(groupPosition).getSublist().get(childPosition)
								.getPoint();
						map.mapMoveToScreenXY(viewid,
								leftMenuAdapter.getGroup(groupPosition).getSublist().get(childPosition).getTitle(),
								location);
					}
				}, delayTime);
